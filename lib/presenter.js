...
// eDNS serves challenge records from a cache with the record's own 300s TTL in
// front of its nameservers. A name being answered for the first time is fast,
// but a *second* value on a name that already has one is invisible until that
// cache expires: measured at 301s against a live zone, against 23-27s for the
// first value. A SAN certificate over example.com and *.example.com puts two
// values on one name, so this is the normal case, not an edge one.
const SHARED_NAME_TIMEOUT = 360000;
...
function create(config) {
	...
	// Waiting for DNS takes minutes, and without a word about it the caller
	// cannot tell a working plugin from a hung one -- which is exactly how it
	// looked in an ioBroker log the first time. Silent unless given somewhere to
	// write, so that nothing shouts into a consumer's stdout uninvited.
	...
	const sharedNameTimeout = options.sharedNameTimeout ?? SHARED_NAME_TIMEOUT;
	...
	async function waitDirectly(servers, host, digest, budget) {
		const started = Date.now();
		let deadline = started + budget;
		let lenientFrom = started + budget / 2;
		let firstRound = true;
		...
		for (;;) {
			...
			if (!answering.length) {
				log(
					`no authoritative nameserver answered directly (${silent.join(', ')}); this host appears to block outbound DNS, falling back to the configured resolver`,
				);
				return { blocked: true, silent };
			}

			// The name already carries values that are not ours, so eDNS is
			// answering from its 300s cache and will not publish ours until that
			// expires. Waiting the ordinary budget would time out on a SAN
			// certificate every single time.
			if (firstRound) {
				firstRound = false;
				sharedName = answering.some(
					result => result.values.length && !result.values.includes(digest),
				);
				if (sharedName && sharedNameTimeout > budget) {
					deadline = started + sharedNameTimeout;
					lenientFrom = started + sharedNameTimeout / 2;
					log(
						`${host} already carries other values, so eDNS publishes an added one only once its 300s record cache expires; waiting up to ${Math.round(sharedNameTimeout / 1000)}s`,
					);
				} else {
					log(
						`waiting up to ${Math.round(budget / 1000)}s for ${host} on ${servers.length} authoritative nameserver(s)`,
					);
				}
			}
			...
			if (
				...
				log(
					`${host} serves the expected value on ${answering.length} of ${results.length} nameserver(s) after ${Math.round((Date.now() - started) / 1000)}s`,
				);
				return { done: true };
			}
	...
	async function waitThroughResolver(zone, host, digest, silent) {
		...
		log(
			`asking the configured resolver for ${host}, first after ${Math.round(fallbackFirstQuery / 1000)}s and then every ${Math.round(interval / 1000)}s: an earlier question would only cache a miss for the zone's SOA minimum`,
		);
		await delay(fallbackFirstQuery);

		for (;;) {
			if ((await systemTxt(host)).includes(digest)) {
				log(`${host} is visible through the configured resolver`);
				return;
			}
	...
	// What is visible at `host` right now. Any authoritative answer counts;
	// where direct queries are impossible the configured resolver is all there
	// is to ask -- the same escalation as the propagation wait, see
	// docs/adr/0004.
	async function visibleValues(zone, host) {
		const results = await pollOnce(await authoritativeServers(zone), host);
		const answering = results.filter(result => !result.error);
		if (answering.length) {
			return answering.flatMap(result => result.values);
		}
		return systemTxt(host);
	}

	async function waitForPropagation(zone, host, digest) {
		const servers = await authoritativeServers(zone);
		const outcome = await waitDirectly(servers, host, digest, propagationTimeout);
		...
		const cause = outcome.sharedName
			? `The name already carries other values, so eDNS is answering from its own 300s record cache and publishes an added value only once that expires. Measured against a live zone this takes about 301s, and the wait above allowed for it.`
		...
		throw new Error(
			`the challenge record for "${host}" (value "${digest}") was not served by all authoritative nameservers of "${zone}" within ${Math.round(outcome.waited / 1000)}s (${outcome.serving} of ${outcome.total} serving it${outcome.silent.length ? `, no answer from ${outcome.silent.join(', ')}` : ''}). ${cause}`,
		);
	}
	...
	const presenter = {
		...
		async get(data) {
			...
			const zone = await zoneFor(host, challenge);
			const values = await visibleValues(zone, host);
